...
class ExtruderCalibrator(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Int32, 'stepper_speed', 10)
    ...
    def extrude(self, distance_mm, speed_sps):
        ...
        Args:
            ...
            speed_sps (int): The speed to run the stepper in steps per second.
        """
        if speed_sps <= 0:
        ...
        self.get_logger().info(f"Calculated steps: {int(total_steps)}")
...
def main(args=None):
    ...
    CALIBRATION_SPEED_SPS = 1000 # A moderate speed in steps per second
